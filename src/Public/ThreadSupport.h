...
namespace Corium::Core {
	...
	public:
		HandleSupport() {
			ThreadHandle defaultHandle = ThreadHandle::getInvalidThread();
			memcpy(m_Storage, &defaultHandle, sizeof(ThreadHandle));
	...
	// ParkingSupport: non-owning alias onto a real handle, not a copy - data()'s address feeds
	// straight into WaitOnAddress/WakeByAddress*, which match by literal memory address.
	...
	public:
		ParkingSupport() = default;

		template<typename H>
		explicit ParkingSupport(H* p_Handle) {
			CORIUM_STATIC_ASSERT(alignof(H) <= CORIUM_SUPPORT_STORAGE_SIZE, "Invalid alignment of incoming handle");
			CORIUM_STATIC_ASSERT(sizeof(H) <= CORIUM_SUPPORT_STORAGE_SIZE, "Incoming handle does not fit in storage");
			m_Storage = reinterpret_cast<std::byte*>(p_Handle);
